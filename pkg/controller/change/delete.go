...
var DeleteCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case deleteFlag.worktree && deleteFlag.index || !deleteFlag.worktree && !deleteFlag.index:
			if !infra.ExistsUntrackedFiles(args...) && !infra.ExistsWorktreeDiff(args...) && !infra.ExistsIndexDiff(args...) {
		...
		case deleteFlag.worktree:
			if !infra.ExistsUntrackedFiles(args...) && !infra.ExistsWorktreeDiff(args...) {
				fmt.Fprintln(os.Stderr, "削除するファイルがありません")
				return
			}
			backupDelete(args...)
...
func deleteWorktree(args ...string) {
	...
	addedList := infra.SearchWorktreeList("A", args[0])
	if len(addedList) != 0 {
...
func deleteIndex(args ...string) {
	indexList := infra.SearchIndexList("", args[0])
	worktreeList := infra.SearchWorktreeList("", args[0])
	indexOnlyList := util.Difference(indexList, worktreeList)
	restoreList := infra.SearchIndexList("a", indexOnlyList...)
	cleanList := infra.SearchIndexList("A", indexOnlyList...)
	if len(indexList) != 0 {
...
func deleteAll(args ...string) {
	...
	restoreList := infra.SearchWorktreeList("a", args[0])
	if len(restoreList) != 0 {
		exitCode := restoreWorktree(restoreList...)
		if exitCode != 0 {
			return
